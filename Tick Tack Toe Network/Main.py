...
rigor = 10000#255168 # Number of training moves

LOAD_A_FILE = True # ARE WE LOADING A MODEL? OR NEW?

WRITE_A_FILE = True #CREATE NEW / Train???

Loading_FileX = "DATA/DictionaryXx.txt" # FROM WHICH FILE? (X and O are for participation award)

Loading_FileO = "DATA/DictionaryOo.txt" # FROM WHICH FILE? (Xx and Oo are for no participation award)

Playing = True # Are we not just training today?

Participation_Award = False #Logical value for rewarding ai for drawing
...
def makemoveashuman(arg, human_first = True):
    ...
    for i in range(3):
        for j in range(3):
            ...
            frame.grid(row=i, column=j, padx=57, pady=67)


            if board[3*i+j]=="N":
                img = NNN
                KAREN = lambda kl=(3*i+j): makemoveashuman(kl, human_first=human_first)
            elif board[3*i+j]=="O":
                img=OOO
                KAREN = None
            ...
            label = tk.Button(image = img, master=frame, text=board[3*i+j], command= KAREN)
            label.grid()
...
if Playing:
    ...
    NNN = ImageTk.PhotoImage(Image.open("N.jpeg"))

    XXX = ImageTk.PhotoImage(Image.open("X.jpeg"))

    OOO = ImageTk.PhotoImage(Image.open("O.jpeg"))


    if not Player_Starts_As_X:
        ai_makes_move_as_x()
    ...
    for i in range(3):
        for j in range(3):
            ...
            if board[3*i+j]=="N":
                img = NNN
                KAREN = lambda kl=(3*i+j): makemoveashuman(kl, human_first=Player_Starts_As_X)
            elif board[3*i+j]=="O":
                img=OOO
                KAREN = None
